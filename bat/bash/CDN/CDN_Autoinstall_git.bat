...
for %%b in (%mdl_all%) do  (
	...
	if !ERRORLEVEL!==0 ( svn export   %%b &&  echo " copy %%b to components  ok" || goto copy_error )  else  (xcopy    %%b  /Y/e "%SPACE%\components\")
)
...
for /f "tokens=1" %%i in (%SPACE%\gitversion.txt) do (
set b=%%i
del  gitversion.txt
)
