...
svn list %svn_dir_path%/%product_version%  --username qiaoting --password 789123
if %errorlevel% EQU 0 goto err_version


set mdl_all=%mdl1_path% %mdl2_path% %mdl3_path% %mdl4_path% %mdl5_path% %mdl6_path% %mdl7_path% %mdl8_path% %mdl9_path% %mdl10_path% %mdl11_path%  %mdl12_path%  %mdl13_path%  %mdl14_path%  %mdl15_path%
echo %mdl_all%
echo %mdl_all%
echo %mdl_all%
echo %mdl_all%
...
for %%a in (%mdl_all%) do (
	echo %%a
	echo !ERRORLEVEL!
	echo "%%a" | findstr /m/i "10.10.5.161 10.10.5.165 10.10.5.66 10.10.5.89"
	IF !ERRORLEVEL!==1 	goto err
	
	echo "%%a" | findstr /m/i "10.10.5.161 10.10.5.165 10.10.5.89"
	IF !ERRORLEVEL!==0 (
	echo "%%a"
	echo hello
	if not exist %%a ( echo [error] %%a  windows ie not exit pls check && exit 1)
	 ) else (
	svn list %%a --username qiaoting --password 789123 
	IF !ERRORLEVEL!==1 ( echo [error] %%a svn path not exit pls check && exit 1))
...
for %%b in (%mdl_all%) do  (
	...
	if !ERRORLEVEL!==0 ( svn copy  %%b %svn_dir_path%/%product_version% -m "%%b test"  --username qiaoting --password 789123 &&  echo "svn copy %%b ok" || goto svn_ci_error )	else (svn import %%b %svn_dir_path%/%product_version%  -m "%%b test" --username qiaoting --password 789123 &&  echo "svn ci %%b ok" || goto svn_ci_error)
)
...
:err
echo [error] package must from 5.165 or 5.161 or 5.89 or http://10.10.5.66(if exit package)
pause
exit 1
