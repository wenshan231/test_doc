...
then 
	...
	if [ "$prj" = "delivery_android_AmS802" ]
	then
		echo ================ current project is delivery_android_AmS802 ================ >> $log_file
		model_list_all=`svn diff  --summarize --username xieyue --password 04303309 -r $lastmmcpver:$nowmmcpver | awk {'print $2'} |  sed 's%/.*$%%' | uniq | grep -v "autoMake" | sed 's/integration//g' | sed 's/tplib/kernel/g' | sed 's/cfg/cfg\/ottdvb_AndroidAmS802/g' | sed 's/thirdparty/thirdparty\/tts thirdparty\/mis thirdparty\/unionPay thirdparty\/ca\/camanager thirdparty\/ca\/caudiadapter/g' | sed 's/porting//g'`           
	  model_list_all="$model_list_all cfg/ottdvb_AndroidAmS802"
	 
	elif [ "$prj" = "delivery_android_AmS802_Client" ]
	then
		echo ================ current project is delivery_android_AmS802_Client ================ >> $log_file	
		model_list_all=`svn diff  --summarize --username xieyue --password 04303309 -r $lastmmcpver:$nowmmcpver | awk {'print $2'} |  sed 's%/.*$%%' | uniq | grep -v "autoMake" | sed 's/integration//g' | sed 's/makefile/kernel/g' | sed 's/tplib/kernel/g' | sed 's/cfg/cfg\/ottdvb_AndroidAmS802_Client/g' | sed 's/thirdparty/thirdparty\/tts thirdparty\/mis  thirdparty\/unionPay thirdparty\/ca\/caudiadapter thirdparty\/ca\/camanager/g' | sed 's/jvm//g' | sed 's/midp//g'`
	  model_list_all="$model_list_all cfg/ottdvb_AndroidAmS802_Client"
	fi
...
then
	...
	then
		rm -fr $workpath/lib
                if [ "$prj" = "delivery_android_AmS802" ]
                then
		        echo ================ current project is delivery_android_AmS802 ================ 
                         
                        model_list_all="dtv kernel protocol codec jsext mediaplayer dtvmx jvm shell guitool graph midp thirdparty/tts thirdparty/mis thirdparty/unionPay thirdparty/ca/camanager thirdparty/ca/caudiadapter cfg/ottdvb_AndroidAmS802"
               
                elif [ "$prj" = "delivery_android_AmS802_Client" ]
                then
		        echo ================ current project is delivery_android_AmS802_Client ================ 
                        
                        model_list_all="dtv kernel protocol codec jsext mediaplayer dtvmx jvm shell guitool graph midp thirdparty/tts thirdparty/mis thirdparty/unionPay thirdparty/ca/camanager thirdparty/ca/caudiadapter cfg/ottdvb_AndroidAmS802_Client"		
		fi			
	fi	
...
cd $allpath/delivery/integration/product/OTT_DVB/porting/Android_AmS802/coship
if [ -d binder_src ]
then
 	rm -rf binder_src
fi
...
then
	...
	if [ "$prj" = "delivery_android_AmS802" ] || [ "$prj" = "delivery_android_AmS802_Client" ]
        then
		product_list="OTT_DVB"
		for productname in $product_list
		do
			cd $workpath/integration/product/$productname
                	if [ "$plat_list" = "Android_AmS802" ]
                	then
				. setupenv --ia setenv_Android_AmS802_debug coship
                	elif [ "$plat_list" = "Android_AmS802_Client" ]
			then                          
                                . setupenv --ia setenv_Android_AmS802_Client_debug coship
                        fi
...
if [ -f $workpath/lib/Android_AmS802/release/libmmcp_ew510*.* ] || [ -f $workpath/lib/Android_AmS802/debug/libmmcp_ew510*.* ] && [ $flag3rdresult -eq 0 ]
then
