...
then 
	...
	then
		...
		model_list_all=`svn diff  --summarize --username xieyue --password 04303309 -r $lastmmcpver:$nowmmcpver | awk {'print $2'} |  sed 's%/.*$%%' | uniq | grep -v "autoMake" | sed 's/integration//g' | sed 's/tplib/kernel/g' | sed 's/cfg/cfg\/jilin_hisense_Android/g' | sed 's/thirdparty/thirdparty\/ca/hisenseci thirdparty\/bankcard thirdparty\/unionPay/g' | sed 's/jvm//g' | sed 's/midp//g' | sed 's/porting//g'`
...
then
	...
	then
		...
		echo ================ current project is jilin:$prj ================ >> $log_file
                model_list="dtvmx shell codec dtv graph jsext kernel mediaplayer midp protocol guitool jvm thirdparty/bankcard thirdparty/ca/hisenseci thirdparty/unionPay porting/share/udi2/lcsp_udi1 porting/share/udi2/ipstack porting/share/udiplus porting/share/udi2/hdicommon/udi2_caadapter cfg stock"
		fi			
	fi	
...
then
	...
	if [ "$prj" = "delivery_android_6a801" ] || [ "$prj" = "delivery_android_6a801_client" ]
		...
		if [ $? -eq 0 ]
		then
			echo -e "================ make  product success =============="
			echo  -e "================ make product success ==============" >> $log_file	
		else
			flag3rdresult=1
			echo -e "================ make  product failed =============="	
			echo  -e "================ make product failed ==============" >> $log_file  
	        	#exit 1
		fi
