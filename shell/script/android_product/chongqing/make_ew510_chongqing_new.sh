...
then 
  ...
  then
	...
	then
		...
		model_list_all=`svn diff  --summarize --username xieyue --password 04303309 -r $lastmmcpver:$nowmmcpver | awk {'print $2'} |  sed 's%/.*$%%' | uniq | grep -v "autoMake" | sed 's/integration//g' | sed 's/tplib/kernel/g' | sed 's/cfg/cfg\/chongqing_coship_Android6A801_Client cfg\/ottdvb_Android6A801_Client/g' | sed 's/thirdparty/thirdparty\/tts thirdparty\/mis thirdparty\/ca\/tfcdcasa/g' | sed 's/jvm//g' | sed 's/guitool//g' | sed 's/graph//g' | sed 's/midp//g' | sed 's/porting//g'`
...
then
	...
	then
		...
		then
			...
			model_list="dtvmx shell codec dtv graph jsext kernel mediaplayer midp protocol guitool jvm thirdparty/tts thirdparty/mis thirdparty/unionPay thirdparty/ca/tfcdcasa cfg/chongqing_coship_Android6A801 cfg/ottdvb_Android6A801"
		...
		then
			...
			model_list="dtv kernel protocol codec jsext mediaplayer dtvmx shell thirdparty/tts thirdparty/mis thirdparty/unionPay thirdparty/ca/tfcdcasa cfg/chongqing_coship_Android6A801_Client cfg/ottdvb_Android6A801_Client"
	...
	if [ "$prj" = "delivery_android_6a801" ] || [ "$prj" = "delivery_android_6a801_client" ]
		...
		then
			...
			then
			 ...
			 ## . setupenv --ia setenv_Android_6A801_release coship
...
##	apk_src=/home/hudson/publisher/binder_510apk/$lastest_package/mstar6a801/Temp/com.coship.mmcp510.merge
	apk_src=$workpath/integration/product/chongqing_android_merge/porting/Android_6A801/coship/apk
	apk_des=$workpath/lib/mmcp_publish
	cp -r $apk_src $apk_des/
	echo -e "================ now, begin to copy apk to mmcp_publish ==============" 
fi
