...
echo "510apk_path:$510apk_src_path" >> $workspace/package/log_version_mmcp.txt
cd $510apk_src_path
if [ $? -eq 0 ]
then
...
then
	...
	cp -f $510apk_src_path/$pakage_version $mmcp_dst_path && echo "copy $pakage_version ok"  >> $workspace/package/log_version_mmcp.txt || echo "copy $pakage_version error"  >> $workspace/package/log_version_mmcp.txt
fi
