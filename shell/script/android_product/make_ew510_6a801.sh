...
then 
	...
	cd $workpath
	if [ "$prj" = "guizhou_Hisence_MTK5505" ]
	then
		echo ================ current project is guizhou_Hisence_MTK5505 ================ >> $log_file
		model_list_all=`svn diff  --summarize --username xieyue --password 04303309 -r $lastmmcpver:$nowmmcpver | awk {'print $2'} |  sed 's%/.*$%%' | uniq | grep -v "autoMake" | sed 's/integration//g' | sed 's/tplib/kernel/g' | sed 's/cfg/cfg\/Hisense_Android_MTK5505/g' | sed 's/thirdparty/thirdparty\/tts thirdparty\/mis thirdparty\/unionPay thirdparty\/ca\/tfcdcasa/g' | sed 's/porting/porting\/share\/udi2\/ipstack porting\/share\/udi2\/hdicommon\/udi2_caadapter porting\/share\/udi2\/lcsp_udi1 porting\/share\/udiplus/g'`

	elif [ "$prj" = "guizhou_android_6a801" ]
	then
	...
	elif [ "$prj" = "wuhan_android_6a801" ]||[ "$prj" = "wuhan_android_6a801_hisense" ]
	then
		echo ================ current project is wuhan_android_6A801 ================ >> $log_file
		model_list_all=`svn diff  --summarize --username xieyue --password 04303309 -r $lastmmcpver:$nowmmcpver | awk {'print $2'} |  sed 's%/.*$%%' | uniq | grep -v "autoMake" | sed 's/integration//g' | sed 's/tplib/kernel/g' | sed 's/thirdparty/thirdparty\/tts thirdparty\/mis thirdparty\/unionPay thirdparty\/ca\/tfcdcasa/g' | sed 's/porting//g'`
	...
	elif [ "$prj" = "chongqing_android_6a801" ]
	then
  		echo ================ current project is chongqing_android_6A801 ================ >> $log_file
		model_list_all=`svn diff  --summarize --username xieyue --password 04303309 -r $lastmmcpver:$nowmmcpver | awk {'print $2'} |  sed 's%/.*$%%' | uniq | grep -v "autoMake" | sed 's/integration//g' | sed 's/tplib/kernel/g' | sed 's/cfg/cfg\/chongqing_coship_Android6A801 cfg\/ottdvb_Android6A801/g' | sed 's/thirdparty/thirdparty\/tts thirdparty\/mis thirdparty\/unionPay thirdparty\/ca\/tfcdcasa/g' | sed 's/porting//g'`
	elif [ "$prj" = "chongqing_android_6a801_client" ]
	then
		echo ================ current project is chongqing_android_6A801_client ================ >> $log_file
		model_list_all=`svn diff  --summarize --username xieyue --password 04303309 -r $lastmmcpver:$nowmmcpver | awk {'print $2'} |  sed 's%/.*$%%' | uniq | grep -v "autoMake" | sed 's/integration//g' | sed 's/tplib/kernel/g' | sed 's/cfg/cfg\/guizhou_coship_Android6A801_Client cfg\/ottdvb_Android6A801_Client/g' | sed 's/thirdparty/thirdparty\/tts thirdparty\/mis thirdparty\/ca\/tfcdcasa/g' | sed 's/jvm//g' | sed 's/guitool//g' | sed 's/graph//g' | sed 's/midp//g' | sed 's/porting//g'`
	fi
	...
	then
		...
		then
			...
			if [ $? -eq 0 ]
			then 
				if [ "$prj" != "guizhou_android_6a801_client" ] && [ "$prj" != "changsha_android_6a801_client" ] && [ "$prj" != "chongqing_android_6a801_client" ]
				then	
...
then
	...
	then
		...
		then
			echo ================ current project is wuhan_android_6A801 ================ >> $log_file
			model_list="dtv kernel protocol codec jsext mediaplayer dtvmx jvm shell guitool graph midp cfg thirdparty/tts thirdparty/mis thirdparty/unionPay "
			
		elif [ "$prj" = "wuhan_android_6a801_client" ]
		then
			echo ================ current project is wuhan_android_6A801_client ================ >> $log_file
			model_list="dtv kernel protocol codec jsext mediaplayer dtvmx shell guitool graph cfg thirdparty/tts thirdparty/mis "
...
binderapk_pubpath=/home/hudson/publisher/binder_510apk_S1003
...
binder_tongfang3_src=/home/hudson/publisher/binder_510apk_S1003/$lastest_package/mstar6a801/Temp/binder/tongfang3
binder_irdeto3_src=/home/hudson/publisher/binder_510apk_S1003/$lastest_package/mstar6a801/Temp/binder/irdeto3
binder_suma_src=/home/hudson/publisher/binder_510apk_S1003/$lastest_package/mstar6a801/Temp/binder/suma
binder_udrm_src=/home/hudson/publisher/binder_510apk_S1003/$lastest_package/mstar6a801/Temp/binder/udrm
   ...
   then
	...
	if [ "$prj" = "delivery_android_6a801" ] || [ "$prj" = "delivery_android_6a801_client" ]
		...
		then
			...
			then
			    echo -e "================ now project is wuhan_android_6a801 =============="
			  . setupenv pf=Android_6A801 manu=coship
			elif [ "$prj" = "wuhan_android_6a801_hisense" ]
			then
			    echo -e "================ now project is wuhan_android_6a801_hisense =============="
			  . setupenv pf=Android_6A801 manu=hisense
			else
				. setupenv --ia setenv_Android_6A801_debug coship
			fi
		elif [ "$plat_list" = "Android_6A801_Client" ]
		...
		elif [ "$prj" = "guizhou_Hisence_MTK5505" ]
		then 
		    . setupenv pf=Hisence_MTK5505 manu=hisense 
		fi
	...
	then
		...
		cd $workpath/dtvmx
		. auto_Ant_Compile.sh
		 cp $workpath/dtvmx/tmp/doc/dtvmx.javadoc $workpath/lib/dtvmx.javadoc -fr
		 cp $workpath/dtvmx/lib $workpath/lib/dtvmxjar -fr
		 cd $workpath/lib/dtvmxjar
		 find -name ".svn" | xargs rm -rf
	fi
	### copy 510apk to mmcp_publish
	apk_src=/home/hudson/publisher/binder_510apk_S1003/$lastest_package/mstar6a801/Temp/com.coship.mmcp510.merge
...
then
		...
		then
			...
			cp /home/hudson/publisher/binder_510apk_S1003/$lastest_package/logs/* $pubdir/log -fr
		else
...
if [ "$prj" = "guizhou_android_6a801_client" ] || [ "$prj" = "chongqing_android_6a801_client" ]
then
...
then
	echo -e "========================== ew510 make success ==========================" >> $log_file
        ...
        echo -e "</infoxml>" >> $log_failedinfo
        exit 0
elif [ -f $workpath/lib/$plat_list/release/libmmcp_ew510*.* ] && [ $flag3rdresult -eq 0 ]
then
	echo -e "========================== ew510 make success ==========================" >> $log_file
        echo -e "Good all make success" >> $log_failedinfo
        echo -e "</infoxml>" >> $log_failedinfo
