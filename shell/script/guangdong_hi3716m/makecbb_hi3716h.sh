...
cd $workpath
echo $workpath
. bin/setupenv --ia bin/envfile_fortrunk/setenv_hi3716H\_$mod
showenv >> $log_file
...
do
	...
	if [ "$mdlcbb" = "cfg" ]
	then
		echo ===================== the current cbb is cfg >> $log_file
		cd $workpath/$mdlcbb
		chmod 777 -Rf *
		make clean > /dev/null 2>&1
		...
		make clean > /dev/null 2>&1
		make config=make3716h_wifi.txt  >> $log_file_detail  2>&1
                if [ $? -ne 0 ]
                then
			flag=1
			rm $workpath/lib/$plat/$mod/libcfgmake3716h_wifi.a -f
			echo ===================== cfg_wifi make failed message is :  ========================
			tail -10 $log_file_detail
                        echo ===================== cfg_wifi make failed message is :  ======================== >> $log_file
                        tail -10 $log_file_detail >>  $log_file
		else
		echo ===================== cfg_wifi make success ========================
			echo ===================== cfg_wifi make success ======================== >> $log_file
                fi

                make clean > /dev/null 2>&1
                make config=make3716h_wifi_shuma.txt  >> $log_file_detail  2>&1
                if [ $? -ne 0 ]
                then
                        flag=1
                        rm $workpath/lib/$plat/$mod/libcfgmake3716h_wifi_shuma.a -f
                        echo ===================== cfg_shumawifi make failed message is :  ========================
                        tail -10 $log_file_detail
                        echo ===================== cfg_shumawifi make failed message is :  ======================== >> $log_file
                        tail -10 $log_file_detail >>  $log_file
                else
                echo ===================== cfg_shumawifi make success ========================
                        echo ===================== cfg_shumawifi make success ======================== >> $log_file
                fi
		...
		if [ "$prj" = "guangdong_hi3716m" ]
	        then
			make clean > /dev/null 2>&1
			make config=make3716m_wifi_dongguang.txt  >> $log_file_detail  2>&1
			if [ $? -ne 0 ]
			then
				rm $workpath/lib/$plat/$mod/libcfgmake3716m_wifi_dongguang.a -f
				echo ===================== cfg_wifi_dongguang for hi3716m make failed message is : ========================
				tail -10 $log_file_detail
				echo ===================== cfg_wifi_dongguang for hi3716m make failed message is : ======================== >> $log_file
				tail -10 $log_file_detail >>  $log_file
			else
							echo ===================== cfg_wifi_dongguang for hi3716m make success ========================
				echo ===================== cfg_wifi_dongguang for hi3716m make success ======================== >> $log_file
			fi


	                make clean > /dev/null 2>&1
			make config=make3716h_jiangmen_wifi.txt  >> $log_file_detail  2>&1
			if [ $? -ne 0 ]
			then
				rm $workpath/lib/$plat/$mod/libcfgmake3716h_jiangmen_wifi.a -f
				echo ===================== cfg_jiangmen_wifi for hi3716m make failed message is : ========================
				tail -10 $log_file_detail
				echo ===================== cfg_jiangmen_wifi for hi3716m make failed message is : ======================== >> $log_file
				tail -10 $log_file_detail >>  $log_file
			else
							echo ===================== cfg_jiangmen_wifi for hi3716m make success ========================
				echo ===================== cfg_jiangmen_wifi for hi3716m make success ======================== >> $log_file
			fi


			
			make clean > /dev/null 2>&1
        make config=make3716m_zhongshan_wifi.txt  >> $log_file_detail  2>&1
        if [ $? -ne 0 ]
        then
                rm $workpath/lib/$plat/$mod/libcfgmake3716m_zhongshan_wifi.a -f
                echo ===================== cfg_zhongshan_wifi for hi3716m make failed message is : ========================
                tail -10 $log_file_detail
                echo ===================== cfg_zhongshan_wifi for hi3716m make failed message is : ======================== >> $log_file
                tail -10 $log_file_detail >>  $log_file
        else
        				echo ===================== cfg_zhongshan_wifi for hi3716m make success ========================
                echo ===================== cfg_zhongshan_wifi for hi3716m make success ======================== >> $log_file
        fi
        
		fi


        elif [	"$mdlcbb" = "thirdparty/mis" ]
	then
		cd $workpath/$mdlcbb
		chmod 777 -Rf *
		echo =====================this is guangdong thirdparty/mis========================
		make clean
        	make > /dev/null 2>&1
		make
        	make config=config_dongguang.txt >> $log_file_detail  2>&1
	elif [	"$mdlcbb" = "cute" ]
	then
